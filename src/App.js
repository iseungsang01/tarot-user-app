...
function Notice({ onBack, customer, onNoticeRead }) {
  ...
  const markReportsAsRead = useCallback(async () => {
    if (!customer) return;
  ...
  }, [customer, myReports, loadMyReports, onNoticeRead]);
  ...
  }, [loadNotices, loadMyReports, markNoticesAsRead, customer]);

  useEffect(() => {
    if (showMyReports && customer) {
      markReportsAsRead();
    }
  }, [showMyReports, customer, markReportsAsRead]);
